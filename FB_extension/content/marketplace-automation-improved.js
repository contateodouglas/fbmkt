...
(function() {
  ...
  class MarketplaceAutomation {
    ...
    getUpdatedSelectors() {
      return {
        ...
        titleInput: 'input[placeholder*="Título" i], input[placeholder*="Title" i], input[aria-label*="título" i], input[aria-label*="title" i]',
        descriptionInput: 'textarea[placeholder*="Descrição" i], textarea[placeholder*="Description" i], textarea[aria-label*="descrição" i], textarea[aria-label*="description" i]',
        priceInput: 'input[placeholder*="Preço" i], input[placeholder*="Price" i], input[aria-label*="preço" i], input[aria-label*="price" i]',
        locationInput: 'input[placeholder*="Localização" i], input[placeholder*="Location" i], input[aria-label*="localização" i], input[aria-label*="location" i]',
        
        // Category selection - Updated for current UI
        categoryDropdown: '[role="combobox"][aria-label*="Categoria" i], [role="combobox"][aria-label*="Category" i], button[aria-label*="Categoria" i], button[aria-label*="Category" i]',
        categoryOption: '[role="option"], [data-visualcompletion="ignore-dynamic"]',
        
        // Condition selection - Updated for current UI
        conditionDropdown: '[role="combobox"][aria-label*="Condição" i], [role="combobox"][aria-label*="Condition" i], button[aria-label*="Condição" i], button[aria-label*="Condition" i]',
        conditionOption: '[role="option"], [data-visualcompletion="ignore-dynamic"]',
        
        // Brand input
        brandInput: 'input[placeholder*="Marca" i], input[placeholder*="Brand" i], input[aria-label*="marca" i], input[aria-label*="brand" i]',
        
        // Image upload - Updated selectors
        imageUploadArea: '[aria-label*="Adicione fotos" i], [aria-label*="Add photos" i], [aria-label*="foto" i], [aria-label*="photo" i]',
        ...
        videoUploadArea: '[aria-label*="Adicione vídeo" i], [aria-label*="Add video" i], [aria-label*="vídeo" i], [aria-label*="video" i]',
        videoUploadInput: 'input[type="file"][accept*="video"]',
        videoPreview: 'video[src*="blob:"], video[src*="scontent"]',
        
        // Tags/Keywords
        tagsInput: 'input[placeholder*="palavra" i], input[placeholder*="keyword" i], input[placeholder*="tag" i], input[aria-label*="tag" i]',
        
        // Navigation buttons - Updated for current UI
        nextButton: '[aria-label*="Avançar" i], [aria-label*="Next" i], [aria-label*="Próximo" i], button:contains("Avançar"), button:contains("Next")',
        publishButton: '[aria-label*="Publicar" i], [aria-label*="Publish" i], button:contains("Publicar"), button:contains("Publish")',
        
        // Error messages
        errorMessage: '[role="alert"], .error, [data-testid*="error"], [aria-live="polite"]',
        
        // Loading indicators
        loadingSpinner: '[role="progressbar"], .loading, [aria-label*="Carregando" i], [aria-label*="Loading" i]',
    ...
    async fillLocation(location) {
      ...
      try {
        ...
        await this.sleep(2000); // Wait for autocomplete suggestions
    ...
    async uploadVideo(video) {
      ...
      try {
        ...
        if (typeof video.dataUrl === 'string' && video.dataUrl.startsWith('data:')) {
          ...
          await this.sleep(this.uploadDelay * 3);
    ...
    async waitForImageUpload(imageIndex = 1) {
      ...
      try {
        ...
        this.log(`Image ${imageIndex} upload completed`);
      } catch (error) {
        this.log(`Image ${imageIndex} upload verification failed:`, error);
    ...
    async waitForVideoUpload() {
      ...
      try {
        ...
        await this.waitForElement(this.selectors.videoPreview, 15000);
        this.log('Video upload completed');
      } catch (error) {
        this.log('Video upload verification failed:', error);
        // Don't throw error, continue
      }
    }
    ...
    async clickElement(element) {
      ...
      this.log('Clicking element:', element.tagName, element.className);
    ...
    /**
     * Clear and fill input with human-like typing
     */
    async clearAndFillInput(input, value) {
      ...
      this.log('Filling input with:', value);
      ...
      // Type the new value character by character (simulate human typing)
     ...
     */
    async base64ToFile(base64Data, fileName, mimeType = 'image/jpeg') {
    ...
    /**
     * Get category mapping for different languages
     */
    getCategoryMap() {
      return {
    ...
    /**
     * Get condition mapping for different languages
     */
    getConditionMap() {
      return {
        'new': 'novo',
    ...
    log(...args) {
      if (this.debugMode) {
        console.log('[Marketplace Automation]', ...args);
      }
    }
  }
  ...
  if (!window.marketplaceAutomation) {
    window.marketplaceAutomation = new MarketplaceAutomation();
  }
